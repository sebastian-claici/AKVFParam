...
double SymmetricDirichlet::compute_energy(const Eigen::MatrixXd& V,
  ...
  for (int i = 0; i < F.rows(); i++) {
    ...
    face_energy.push_back(l_part * r_part);
  ...
  if (norm == 2) // l2 norm
    return std::accumulate(face_energy.begin(), face_energy.end(), 0.0);
  else if (norm == -1) // l_inf norm
    return *std::max_element(face_energy.begin(), face_energy.end());
}
