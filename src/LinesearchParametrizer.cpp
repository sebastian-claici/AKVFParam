...
double LinesearchParametrizer::parametrize( const Eigen::MatrixXd& V, const Eigen::MatrixXi& F,
  ...
  double e = line_search(V,F,uv,d,min_step_to_singularity, energy, cur_energy);
...
double LinesearchParametrizer::line_search(const Eigen::MatrixXd& V, const Eigen::MatrixXi& F,
  ...
  bool ret = false;
  if (!type) {
    ret = bisection_wolfe_conditions_search(V,F, energy, d, min_step_to_singularity, uv, old_energy,
                                                 linesearch_uv, linesearch_e);

  } else {
    ret = backtracking_wolfe_conditions_search(V,F, energy, d, min_step_to_singularity, uv, old_energy,
                                                    linesearch_uv, linesearch_e);
  }
...
bool LinesearchParametrizer::backtracking_wolfe_conditions_search(const Eigen::MatrixXd &V,
  ...
  double min_alpha = 0;
  double step_size = min(1., search_rho * min_step_to_singularity);
